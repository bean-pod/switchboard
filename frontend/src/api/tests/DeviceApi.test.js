...
describe("DeviceApi", () => {
    ...
    describe("getSenders", () => {
        it("Should call axios and return the senders", done => {
            ...
            jest.spyOn(global.Date, "now")
                .mockImplementationOnce(() => new Date(`${sampleSendersResponse[0].lastCommunication}Z`));
    ...
    describe("getReceivers", () => {
        it("Should call axios and return the receivers", done => {
            ...
            jest.spyOn(global.Date, "now")
                .mockImplementationOnce(() => new Date(`${sampleReceiversResponse[0].lastCommunication}Z`));

            DeviceApi.getReceivers((result) => {
                try {
                    expect(axios.get).toHaveBeenCalledWith("http://localhost:8080/decoder");
