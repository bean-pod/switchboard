...
const expectedFirstStreamRespone = new StreamInfo(1, convertToDataObject(axiosGetFirstStreamResponse.outputChannel.encoder), convertToDataObject(axiosGetFirstStreamResponse.inputChannel.decoder), ["Additional stream info goes here."]);
const expectedSecondStreamRespone = new StreamInfo(2, convertToDataObject(axiosGetSecondStreamResponse.outputChannel.encoder), convertToDataObject(axiosGetSecondStreamResponse.inputChannel.decoder), ["Additional stream info goes here."]);

const expectedAllStreamsResponse = [expectedFirstStreamRespone, expectedSecondStreamRespone];
...
describe("Stream Api", () => {
  ...
  describe("getAllStreams", () => {
    it("should call axios.get and return an array of streams", async () => {
      ...
      axios.get.mockResolvedValueOnce({data: axiosAllStreamsResponse})
